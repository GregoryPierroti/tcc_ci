...
Tag de referência: `baseline-ci-v1`.

Este documento registra o estado reproduzível dos três objetos experimentais
...
- As verificações locais são expostas por `make format-check`, `make lint`,
  `make test` e, nos objetos Python, `make security`; as pipelines GitHub
  Actions chamam esses mesmos comandos.
...
7. os três workflows GitHub Actions passam no commit de referência.
