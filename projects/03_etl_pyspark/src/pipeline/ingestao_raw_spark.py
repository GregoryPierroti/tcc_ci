...
class IngestaoRawSpark:
    ...
    def executar(self):
        ...
        def bancos_reader(path):
            return (
                self.spark.read.format("csv")
                .schema(schema_bancos)
                .option("header", True)
                .option("delimiter", "\t")
                .load(path)
            )
        ...
        schema_reclamacoes = StructType(
            [
                ...
                StructField("Quantidade de reclamacoes reguladas procedentes", IntegerType(), True),
                StructField("Quantidade de reclamacoes reguladas - outras", IntegerType(), True),
                StructField("Quantidade de reclamacoes nao reguladas", IntegerType(), True),
                StructField("Quantidade total de reclamacoes", IntegerType(), True),
                StructField("Quantidade total de clientes CCS e SCR", LongType(), True),
                StructField("Quantidade de clientes CCS", LongType(), True),
                StructField("Quantidade de clientes SCR", LongType(), True),
                StructField("campo_extra", StringType(), True),
            ]
        )

        def reclamacoes_reader(path):
            return (
                self.spark.read.format("csv")
                .schema(schema_reclamacoes)
                .option("header", True)
                .option("delimiter", ";")
        ...
        self.ler_e_salvar_parquet(
            reclamacoes_reader, "CSV", caminho_reclamacoes, "reclamacoes_parquet"
        )

        caminho_empregados = os.path.join(
            self.base_dir, "Empregados", "glassdoor_consolidado_join_match_less_v2.csv"
        )
        schema_empregados = StructType(
            [
                StructField("employer_name", StringType(), True),
                StructField("reviews_count", IntegerType(), True),
                StructField("culture_count", IntegerType(), True),
                StructField("salaries_count", IntegerType(), True),
                StructField("benefits_count", IntegerType(), True),
                StructField("employer-website", StringType(), True),
                StructField("employer-headquarters", StringType(), True),
                StructField("employer-founded", IntegerType(), True),
                StructField("employer-industry", StringType(), True),
                StructField("employer-revenue", StringType(), True),
                StructField("url", StringType(), True),
                StructField("Geral", DoubleType(), True),
                StructField("Cultura e valores", DoubleType(), True),
                StructField("Diversidade e inclusão", DoubleType(), True),
                StructField("Qualidade de vida", DoubleType(), True),
                StructField("Alta liderança", DoubleType(), True),
                StructField("Remuneração e benefícios", DoubleType(), True),
                StructField("Oportunidades de carreira", DoubleType(), True),
                StructField("Recomendam para outras pessoas(%)", StringType(), True),
                StructField("Perspectiva positiva da empresa(%)", StringType(), True),
                StructField("CNPJ", StringType(), True),
                StructField("Nome", StringType(), True),
                StructField("match_percent", DoubleType(), True),
            ]
        )

        def empregados_reader(path):
            return (
                self.spark.read.format("csv")
        ...
        self.ler_e_salvar_parquet(
            empregados_reader, "CSV", caminho_empregados, "empregados_parquet"
        )
